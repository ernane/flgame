...
def tainNetwork():
    ...
    while "ticky" != "tacky":
        ...
        run_time += time_diff
        print(
            "Iteração {} completada com {} wins, {} losses {} draws, out of {} games played, e is {} \ncost is {} , current_time is {}, time taken is {} , total time = {} hours \n".format(
                iterations,
                won_games,
                lost_games,
                draw_games,
...
def playaGame(e, sess, inputState, prediction, Qoutputs):
    ...
    while True:
        ...
        selectedRandomIndex = random.choice(zero_indexes)
        pred, _ = sess.run(
            [prediction, Qoutputs], feed_dict={inputState: [temp_copy_inverse]}
        )
        isFalsePrediction = False if temp_copy[pred] == 0 else True
